...
function OtpForm() {
  ...
  const handleVerify = async () => {
    ...
    if (otpCode.length < 4) { toast.error('Enter 4-digit code'); return; }
  ...
  return (
    <>
      <div className="text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">Enter the 4-digit security code</p>
      </div>
      <div className="flex justify-center gap-3">
        {code.map((digit, i) => (
          <Input key={i} id={'otp-'+i} type="text" maxLength={1} value={digit}
            onChange={e => handleChange(i, e.target.value)}
            className="h-16 w-16 text-center text-2xl font-bold rounded-xl border-2" />
        ))}
      </div>
      <Button onClick={handleVerify} className="w-full bg-gradient-to-r from-blue-600 to-purple-600" disabled={isSubmitting}>
...
export default function VerifyOtpPage() {
  return (
    <div className="space-y-6">
      <Link href="/login" className="flex items-center gap-1 text-sm text-muted-foreground">
